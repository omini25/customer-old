...
interface Props {
  ...
  device: any;
  closeModal: () => void;
  setDeviceRecovered: any;
}

const RecoverDeviceModal: FC<Props> = ({ isOpen, closeModal, setDeviceRecovered, device }) => {
  return (
          ...
          >
            <div className="my-4 inline-block transform overflow-hidden bg-daabo-white py-10 px-5 text-left align-middle shadow-xl transition-all md:my-8 md:py-10 md:px-10 lg:w-[40.500rem] lg:py-[3.313rem]">
              ...
              <Dialog.Description className="mx-auto mt-[1rem] max-h-[40.500rem] max-w-[29.375rem] text-center text-[0.78rem] md:text-[0.875rem]">
                By clicking the button below, Daabo would stop sending reports and will go back to
                normal status.
              </Dialog.Description>
              ...
              <div className="mt-8 flex items-center space-x-2 lg:mt-[3.125rem] lg:space-x-4">
                <button
                  className="mx-auto h-[2.750rem] w-[14.125rem] rounded-md border-2 border-[#4CAF50] bg-white px-4 text-xs font-medium text-[#4CAF50] hover:bg-[#4CAF50] hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 md:text-sm"
                  onClick={() => setDeviceRecovered(device?.ID)}
                >
                  DEVICE RECOVERED
                </button>
              </div>
...
export default RecoverDeviceModal;
